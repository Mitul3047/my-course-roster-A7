...
const Cart = ({ selectCourse, hourRemain, totalHour, totalPrice, handleRemoveCourse }) => {
    ...
    return (
        <div className="bg-base-100 w-full p-6 rounded-lg shadow-xl">
            ...
            <h3 className="text-[#1C1B1B] text-xl font-bold pt-4 pb-5">Course Name</h3>
            {
                selectCourse.map(course => (
                    <li key={course.id} className="list-decimal  text-[#1C1B1B99]">
                        {course.title}
                        <span className="text-lg cursor-pointer  text-red-700" onClick={() => handleRemoveCourse(course)}>
                            <TiDelete />
                        </span>
                    </li>
                ))
